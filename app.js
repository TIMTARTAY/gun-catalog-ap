...
let state={firearms:[],ammo:[],maintenance:[],settings:{theme:'light',pinHash:'',migrated:false}};
...
window.deleteFirearm=id=>{if(confirm('Delete this firearm and its linked records?')){state.firearms=state.firearms.filter(x=>x.id!==id);state.maintenance=state.maintenance.filter(x=>x.firearmId!==id);save();toast('Firearm deleted.')}};
...
$('firearmForm').onsubmit=e=>{e.preventDefault();const id=$('firearmId').value||uid(),old=state.firearms.find(f=>f.id===id);const f={id,type:$('fType').value,condition:$('fCondition').value,make:$('fMake').value.trim(),model:$('fModel').value.trim(),serial:$('fSerial').value.trim(),caliber:$('fCaliber').value.trim(),purchaseDate:$('fPurchaseDate').value,purchasePrice:Number($('fPurchasePrice').value||0),value:Number($('fValue').value||0),location:$('fLocation').value.trim(),source:$('fSource').value.trim(),status:$('fStatus').value,notes:$('fNotes').value.trim(),accessories:structuredClone(accessoryDraft),photos:structuredClone(photoDraft),docs:structuredClone(docDraft),createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};const duplicate=state.firearms.find(x=>x.id!==id&&f.serial&&x.serial.toLowerCase()===f.serial.toLowerCase());if(duplicate&&!confirm('Another item uses this serial number. Save anyway?'))return;const i=state.firearms.findIndex(x=>x.id===id);i>=0?state.firearms[i]=f:state.firearms.push(f);save();closeModal('firearmModal');toast('Firearm saved.')};
...
window.deleteAmmo=id=>{if(confirm('Delete this ammunition record?')){state.ammo=state.ammo.filter(x=>x.id!==id);save()}};
$('ammoForm').onsubmit=e=>{e.preventDefault();const id=$('ammoId').value||uid(),a={id,caliber:$('aCaliber').value.trim(),brand:$('aBrand').value.trim(),type:$('aType').value.trim(),qty:Number($('aQty').value),lot:$('aLot').value.trim(),location:$('aLocation').value.trim(),date:$('aDate').value,cost:Number($('aCost').value||0),notes:$('aNotes').value.trim(),updatedAt:new Date().toISOString()};const i=state.ammo.findIndex(x=>x.id===id);i>=0?state.ammo[i]=a:state.ammo.push(a);save();closeModal('ammoModal');toast('Ammunition saved.')};
...
window.deleteMaintenance=id=>{if(confirm('Delete this maintenance record?')){state.maintenance=state.maintenance.filter(x=>x.id!==id);save()}};
$('maintenanceForm').onsubmit=e=>{e.preventDefault();const id=$('mId').value||uid(),m={id,firearmId:$('mFirearm').value,type:$('mType').value,date:$('mDate').value,due:$('mDue').value,rounds:Number($('mRounds').value||0),cost:Number($('mCost').value||0),by:$('mBy').value.trim(),notes:$('mNotes').value.trim(),updatedAt:new Date().toISOString()};const i=state.maintenance.findIndex(x=>x.id===id);i>=0?state.maintenance[i]=m:state.maintenance.push(m);save();closeModal('maintenanceModal');toast('Maintenance record saved.')};
